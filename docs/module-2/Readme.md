...
| **Métrica**                   | **Cálculo**                                                                 | **Caso de Uso**                                                                                                 |
|-------------------------------|-----------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------|
| **Lead Time to Change**       | Tiempo desde la solicitud de cambio hasta su despliegue en producción.       | Medir la velocidad de entrega de nuevas características y mejoras en el sistema.                                |
| **Deployment Frequency**      | Número de despliegues en producción en un periodo determinado.              | Evaluar la frecuencia de actualizaciones y mejoras del sistema en producción.                                    |
| **Mean Time to Recovery (MTTR)** | Tiempo promedio para recuperar el sistema después de una falla.             | Evaluar la eficiencia de los procesos de recuperación tras incidentes o fallos.                                  |
| **Change Failure Rate**       | (Número de fallos / Número total de cambios) * 100                           | Identificar la estabilidad del proceso de despliegue y la calidad de los cambios implementados.                 |
| **Logs**                      | Datos capturados sobre eventos en el sistema a lo largo del tiempo.         | Monitorear el comportamiento del sistema, detectar problemas y analizar la actividad de los usuarios.            |
| **Traces**                    | Registro detallado del flujo de ejecución de las solicitudes en el sistema. | Analizar y resolver problemas de rendimiento, especialmente en arquitecturas de microservicios.                 |
| **Metrics**                   | Medidas numéricas capturadas y registradas regularmente.                    | Monitorear el rendimiento del sistema (por ejemplo, uso de CPU, memoria, etc.) para optimización continua.       |
| **Chaos**                     | No aplica - falta de recolección de datos estructurada.                     | Situación inicial donde no se recopilan métricas, llevando a falta de información para decisiones.               |
| **Defined Data**              | Identificación de datos clave pero sin uso regular.                        | Identificar métricas importantes pero aún no integradas en los procesos de revisión.                             |
| **Repeatable Data**           | Uso constante de datos en dashboards o métricas.                            | Evaluar el uso regular de datos para la toma de decisiones y la supervisión de procesos.                         |
| **Managed**                   | Integración de métricas en la toma de decisiones.                           | Utilizar dashboards y métricas para guiar la mejora continua y la gestión del rendimiento.                       |
| **Optimized**                 | Uso avanzado de métricas para optimización continua.                        | Ajustar y mejorar procesos basándose en análisis avanzados de métricas y feedback automatizado.                  |
| **Test Success Ratios**       | (Pruebas exitosas / Pruebas totales) * 100                                  | Evaluar la calidad del software y la efectividad de los tests automatizados.                                     |
| **Root Cause for Failure**    | Tiempo para identificar y corregir la causa raíz de un fallo.               | Medir la eficiencia en la resolución de problemas críticos y la mejora de la resiliencia del sistema.            |
| **Training Rate**             | Porcentaje de miembros del equipo que completan una formación específica.   | Evaluar la adopción y efectividad de las capacitaciones en nuevas tecnologías o prácticas de seguridad.          |
| **Scan Coverage**             | (Código escaneado / Código total) * 100                                     | Evaluar la cobertura de los escaneos de seguridad y la detección de vulnerabilidades en el código base.          |
